...
function main() {
    ...
    function drawCircle() {
        ...
        let numTriangles = 50
        let h = 2 * Math.PI / numTriangles;
        for (i = 0; i < numTriangles; i++) {
            p1 = vec2.fromValues(Math.cos(h * i) * .2, Math.sin(h * i) * .2);
            p2 = vec2.fromValues(Math.cos(h * (i + 1)) * .2, Math.sin(h * (i + 1)) * .2);
    ...
    function drawTriangle() { 
        ...
        let p2 = vec2.fromValues(Math.cos(.33 * Math.PI + globalTime), Math.sin(.33 * Math.PI + globalTime));
        if (false) {
            p0 = vec2.fromValues(Math.cos(globalTime), Math.sin(globalTime));
            p1 = vec2.fromValues(Math.cos(-.33 * Math.PI + globalTime) * .5, Math.sin(-.33 * Math.PI + globalTime)* .5);
            p2 = vec2.fromValues(Math.cos(.33 * Math.PI + globalTime)* .5, Math.sin(.33 * Math.PI + globalTime)* .5);
        }
        points.push(p0);
        points.push(p1);
        points.push(p2);
    ...
    function drawUpsiTriangle() { 
        // {points} is an array of gLmatrix.vec2's
        let points = [];
        let p0 = vec2.fromValues(Math.cos(Math.PI + globalTime + 1/6 * Math.PI), Math.sin(Math.PI + globalTime + 1/6 * Math.PI));
        let p1 = vec2.fromValues(Math.cos(-1.33 * Math.PI + globalTime + 1/6 * Math.PI) * .5, Math.sin(-1.33 * Math.PI + globalTime + 1/6 * Math.PI)* .5);
        let p2 = vec2.fromValues(Math.cos(1.33 * Math.PI + globalTime + 1/6 * Math.PI)* .5, Math.sin(1.33 * Math.PI + globalTime + 1/6 * Math.PI)* .5);
        points.push(p0);
        points.push(p1);
        points.push(p2);
