...
const phongVertexShaderSource = `
  ...
  void main(){
      ...
      vec4 vPos4 = uModelViewMatrix * vec4(vPosition, 1.0);
...
const phongFragmentShaderSource = `
  ...
  void main(void) {
      ...
      vec3 V = normalize(-vpos); // since camera is at pos 0,0,0?

      vec3 R = (2.0 * dot(L, N)) * N - L;

      // combine to find lit color
      vec3 litColor = ambient + diffuse * max(dot(L, N), 0.) + specular * pow(max(dot(R, V), 0.), alpha); 
...
const phongTexFragmentShaderSource = `
  ...
  void main() {
      ...
      vec3 diffuse = 0.7*texColor.rgb; 

      vec3 specular = specColor;
      
      vec3 L = normalize(lightPos - vertPos);
      vec3 N = normalize(normal);
      vec3 V = normalize(-vertPos); // since camera is at pos 0,0,0?
      vec3 R = (2.0 * dot(L, N)) * N - L;
